...
@section('meta')
    ...
    <link rel="canonical" href="{{ url('/') }}/p/{{$subPabble->name}}/comments/{{$thread->code}}/{{ str_slug($thread->title) }}">
    <style amp-custom>
        @php
...
@section('content')
    ...
    <div class="container">
        <div id="content_wrapper" class="row">
            <div id="subpabble_name">
                <a href="/p/{{$subPabble->name}}">/p/{{$subPabble->name}}</a>
            </div>
            <div id="title">
                <h1><a href="{{ url('/') }}/p/{{$subPabble->name}}/comments/{{$thread->code}}/{{ str_slug($thread->title) }}">{{$thread->title}}</a></h1>
            </div>
            <div id="poster_date">
    ...
    <div class="commentsHeader">
        ...
        <div class="container">
            <div class="row">
                ...
                <a href="{{ url('/') }}/p/{{$subPabble->name}}/comments/{{$thread->code}}/{{ str_slug($thread->title) }}" class="button_more"><span>View more comments</span></a>
            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            ...
            @if($top_posts->count() > 0)
                <div id="title">
                    <h1 class="center padding-top">Top posts in <a href="/p/{{$subPabble->name}}">/p/{{$subPabble->name}}</a></h1>
                </div>
                @foreach($top_posts as $thread)
                    <div class="post row">
                        ...
                        <div id="post_wrapper" class="col-9-sm">
                            <div class="title">
                                <a href="/p/{{$subPabble->name}}/comments/{{$thread->code}}/{{str_slug($thread->title)}}">{{$thread->title}}</a>
                            </div>
                        </div>
                    </div>
                @endforeach
            @endif
            <a href="{{ url('/') }}/p/{{$subPabble->name}}" class="button_more">View more /p/{{$subPabble->name}} posts</a>
        </div>
    </div>
@endsection
