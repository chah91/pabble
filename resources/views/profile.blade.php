                            ...
                            <div style="margin-top:0; padding-bottom: 10px; margin-bottom: 10px;" class="panel">
                                <div class="thread">
                                    ...
                                    <div style="min-width: 90px;" class="image col-xs-1">
                                        <div class="row">
                                            <a href="@if($post->link) {{$post->link}} @else {{url('/')}}/p/{{$subpabble->name}}/comments/{{$post->code}}/{{$post->title}} @endif"><img style="max-height: 76px; max-width: 76px;" src="@if($post->thumbnail !== null){{$post->thumbnail}} @elseif($post->link) {{url('/')}}/images/link_thumb.png @else {{url('/')}}/images/text_thumb.png @endif" alt="{{$post->title}}"></a>
                                        </div>
                                    </div>
                                    <div class="thread_info">
                                        <a style="color: #636b6f;" href="@if($post->link) {{$post->link}} @else {{url('/')}}/p/{{$subpabble->name}}/comments/{{$post->code}}/{{str_slug($post->title)}} @endif"><h3 class="thread_title overflow">{{$post->title}}</h3></a>
                                        ...
                                        <a href="{{url('/')}}/p/{{$subpabble->name}}/comments/{{$post->code}}/{{$post->title}}"><p class="overflow" style="margin-top: -10px;"><strong>{{$post->reply_count}} {{str_plural('reply', $post->reply_count)}}</strong></p></a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($comments as $comment)
                            ...
                            <div id="post_panel_{{$comment->id}}" style="width:100%; min-width: 400px; padding:10px; margin-bottom: 10px;" class="col-xs-12 panel comment">
                                ...
                                <div class="col-xs-10 col-sm11">
                                    ...
                                    <div style="margin-bottom:3px;" class="linkwrapper"><a style="color: grey;" href="/p/{{$subpabble->name}}/comments/{{$thread->code}}">thread</a></div>
