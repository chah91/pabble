...
class HomeController extends Controller
{
    ...
    public function index(Request $request, Thread $thread, Vote $vote, Subscription $subscription)
    {
        ...
        if (Auth::check() && $sort_type !== 'g') {
            ...
            $subscriptionsIdsArray = $subscription->select('sub_pabble_id')->where('user_id', $user->id)->get()->toArray();
        }
        ...
        if (!$sort) {
            if ($user) {
                $threads = $thread->whereIn('sub_pabble_id', $subscriptionsIdsArray)->where('created_at', '>=', \Carbon\Carbon::now()->subDay(7))->take(25)->orderBy('score', 'DESC');
            } else {
            ...
            if ($user) {
                $threads = $thread->whereIn('sub_pabble_id', $subscriptionsIdsArray)->orderBy('created_at', 'DESC')->take(25);
            } else {
                $threads = $thread->orderBy('created_at', 'DESC')->take(25);
            }
        } else if ($sort == 'top') {
            if ($user) {
                $threads = $thread->whereIn('sub_pabble_id', $subscriptionsIdsArray)->orderBy('score', 'DESC')->take(25);
            } else {
